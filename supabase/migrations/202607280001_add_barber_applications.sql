...
for each row
execute function public.set_barber_application_updated_at();
...
drop trigger if exists create_barber_application_after_signup
  on auth.users;

create trigger create_barber_application_after_signup
after insert on auth.users
for each row
execute function public.create_barber_application_for_new_user();

create or replace function public.approve_barber_application(
  application_id uuid
)
...
begin
  ...
  set role = 'barber',
      full_name = coalesce(nullif(application.full_name, ''), full_name),
      email = coalesce(nullif(application.email, ''), email),
      phone = coalesce(nullif(application.phone, ''), phone),
      updated_at = now()
  ...
  generated_slug :=
    lower(
      trim(
        both '-'
        from regexp_replace(
          coalesce(nullif(application.full_name, ''), 'barber'),
          '[^a-zA-Z0-9]+',
          '-',
          'g'
        )
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'square_connections_barber_id_fkey'
      and conrelid = 'public.square_connections'::regclass
  ) then
    alter table public.square_connections
      add constraint square_connections_barber_id_fkey
